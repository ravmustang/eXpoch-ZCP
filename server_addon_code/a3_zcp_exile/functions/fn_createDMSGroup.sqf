...
private _ZCP_CDG_dummyGroup = createGroup (missionNamespace getVariable [format ["ZCP_CONFIG_AI_%1Side",_ZCP_CDG_side],"bandit"]);

for "_i" from 1 to _ZCP_CDG_unitsPerGroup do {
  private _ZCP_unit = [_ZCP_CDG_dummyGroup, _ZCP_CDG_spawnAIPos, _ZCP_CDG_dificulty, _ZCP_CDG_solierType, _ZCP_CDG_radius, _ZCP_CDG_side] call ZCP_fnc_createDMSSoldier;
};

sleep 0.2;

private _ZCP_CDG_groupAI = createGroup (missionNamespace getVariable [format ["ZCP_CONFIG_AI_%1Side",_ZCP_CDG_side],"bandit"]);
...
_ZCP_CDG_groupAI setVariable ["DMS_Group_Side", _ZCP_CDG_side];
