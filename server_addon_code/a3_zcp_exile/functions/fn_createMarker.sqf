...
private _ZCP_CM_missionAIArray = _ZCP_CM_captureObject select 23;
private _ZCP_CM_missionAISide = _ZCP_CM_missionAIArray select 4;
...
_ZCP_CM_attentionMarker 		setMarkerColor (missionNamespace getVariable [format ["ZCP_Background_%1Color",toLower(_ZCP_CM_missionAISide)],"bandit"]);
...
(
	switch ( (ZCP_Data select _ZCP_CM_index) select 1 ) do
	{
		...
		default
		{
			(missionNamespace getVariable [format ["ZCP_%1FreeColor",toLower(_ZCP_CM_missionAISide)],"bandit"])
		};
	}
);
